...
def create_text(text, font_name="IBMPlexMono-Regular.ttf", font_size=20):
    ...
    for line in text:
        lines.append(line)
    logo = PIL.Image.open("logo.png", "r").resize((142, 72))
    height = 30 + (len(lines) * 23 + logo.size[1])
    lines = "\n".join(lines)

    img = PIL.Image.new('RGB', (PrinterWidth, height), color = (255, 255, 255))
    font = PIL.ImageFont.truetype(font_name, font_size)
    img.paste(logo, (156, 71))
    d = PIL.ImageDraw.Draw(img)
    d.multiline_text((0, 177), lines, fill=(0,0,0), font=font, spacing=-10)
